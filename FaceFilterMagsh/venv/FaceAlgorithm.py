...
def edge_detection(pixels):
    ...
    averages_grid = t = [ [0]*ROWS for i in range(COLS)]
    counter = 1
    row_counter = 0
    col_counter = 0
    while start_row != ROWS:
        ...
        #grid_avg = grid[0][0]#get_grid_avg(grid, MINI_GRID_SIDE, MINI_GRID_SIDE)
        averages_grid[row_counter][col_counter] = grid[0][0]
        counter = counter + 1
        if (end_col == COLS):
            start_row += 1
            end_row += 1
...
def main():
    ...
    img = resize_img(test_12, ROWS, COLS)
    imgGray = greyscale(img)
    pixels = img_to_array(imgGray)
    #grid = grid_array(start_row, end_row, start_col, end_col, pixels)
    #rows, cols = get_image_size(imgGray)

    averages_grid = edge_detection(pixels)
